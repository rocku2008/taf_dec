...
def count_val(source, target, key_columns):
    ...
    if source_cnt == target_cnt:
        ...
        write_output(validation_type="count check",
                     status=status,
                     details=f"Count is matching between source and target. source count is {source_cnt} and target count is {target_cnt}")
        ...
        print(f"count is not matching. source count is {source_cnt} and target count is {target_cnt} and diff is {abs(source_cnt-target_cnt)}")
