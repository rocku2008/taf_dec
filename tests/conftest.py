...
#     return spark
#
# #once we define the fixture then we are calling in the following functions for example in read_data.
#
#
# @pytest.fixture(scope='module')
# def read_config(request):
...
#         creds = yaml.safe_load(f)['qa']
#     return creds
#
# #In azure there is an option called Azure vault.  In databricks we can use databricks secrets to store credentials.Since we are currently wokring on local, we are creating
# #credentail_file to store  under the tests folder.
#
# def read_db(spark,config_data,dirpath):
# #in this function we are passing normal variables in the arguments coz this fuction is not a fixture
#     creds = read_cred(dirpath)
# # we need to pass cred in inside if and outside if so we need to read cred file here.
#     if config_data['transformation'][0].lower() == 'y' and config_data['transformation'][1].lower() == 'sql':
...
#
#
# # def read_file(spark,config_data,dir_path=None):
# #     if config_data['type'] == 'csv':
# #         if config_data['schema'] == 'Y':
# #             schema = read_schema(dir_path)
# #             df = spark.read.schema(schema).csv(config_data['path'], header=config_data['options']['header'])
